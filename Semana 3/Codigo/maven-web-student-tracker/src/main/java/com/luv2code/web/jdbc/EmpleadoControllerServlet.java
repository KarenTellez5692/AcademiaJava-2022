...
public class EmpleadoControllerServlet extends HttpServlet {
	...
	private EmpleadoDbUtil studentDbUtil;
	
	@Resource(name="jdbc/web_Empleado_tracker") //SE COMENTO PARA HACER USO DE JNDI
	...
	public void init() throws ServletException {
		...
		try {
			...
			studentDbUtil = new EmpleadoDbUtil(dataSource);
		}
		catch (Exception exc) {
	...
	private void updateStudent(HttpServletRequest request, HttpServletResponse response)
		...
		Empleado theStudent = new Empleado(id, firstName, lastName, email,area);
	...
	private void addStudent(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		// create a new student object
		Empleado theStudent = new Empleado(firstName, lastName, email, area);
	...
	private void listStudents(
		...
		List<Empleado> empleados = studentDbUtil.getStudents();
		
		for (Empleado s:empleados) {
			System.out.println(s);
		}
		
		empleados.add(new Empleado(999, "FirstName","lastName", "email@gmail", "areaGenerica"));
		// add students to the request
		request.setAttribute("LISTA_ESTUDIANTES", empleados);
